...
plt.figure()
sns.boxplot(data=ttlonmeasures, x='applied_current group', y='baselinev', hue='drug_condition')
...
# %% opto response sensitivity to light intensity - TODO: separate out by holding level/baselineV
# getting ttlonmeasures_df for non-drug runs only:
ttlonmeasures_sthr_nodrug = ttlonmeasures_sthr[~ttlonmeasures_sthr.file_origin.str.contains('uinpirole')]
...
for level in intensity_levels:
    ...
    neuron_data.plot_ttlaligned(ttlonmeasures_nodrug_atlevel, plt_title=('intensity=' + str(level)),
                                ...
                                color_lims=[-90, -55],
                                plotlims=[-1, 10, -0.01, 0.1]
                                )
# making a boxplot of response amplitudes by light intensity levels:
plt.figure()
sns.boxplot(x='stim_intensity_pct', y='response_maxamp', data=ttlonmeasures_sthr_nodrug)

# opto response amplitude clearly increases with stim.intensity

# %% opto response sensitivity to drug - TODO: separate out by holding level/baselineV
...
for condition in drug_conditions:
    ...
    neuron_data.plot_ttlaligned(ttlonmeasures_sthr_100pct_incondition, plt_title=condition,
                                prettl_t_inms=5, postttl_t_inms=70,
                                color_lims=[-90, -55],
                                plotlims=[-1, 12, -0.01, 0.1]
                                )


# making a boxplot of response amplitudes by drug conditions & baselineV/holding level:
plt.figure()
sns.boxplot(x='drug_condition', y='response_maxamp', data=ttlonmeasures_sthr_100pct)
