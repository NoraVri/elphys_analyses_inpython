...
# gradual shift down in holdingC stabilizing for the last two recording blocks - looks like it could be
# consistent with drug effect
...
# Looks like all except 2 responses to optostim are clustered very tightly together in measured parameters
# One response measured as peaking ~80ms post ttl (all others <10ms) is because of some weird noise-thing
# in the recording at that time; the actual response looks good though.
## Re-running get_singlepulse_ttlonmeasures_fromrawdata with 50ms time window to avoid this
# This looks good: by amplitude, one response was an APcurrent (amp~8nA, all other response amps <600pA).

# %% Getting subthreshold responses only:
ttlonmeasures_sthr = ttlonmeasures[ttlonmeasures.response_maxamp < 1000]
...
# Interesting: all responses seem to have a double up-stroke, with a plateau after the first that is much
# more consistent in amplitude than the peak that follows the second up-stroke (and the plateau-level may be different
...
# This dataset consists of 499 recorded subthreshold responses.
...
# Adding drug conditions as a column to ttlonmeasures dataframe: (from Notes0530.rtf, cell 1)
...
# This all looks like drug has no effect at all.
