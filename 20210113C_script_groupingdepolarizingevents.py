...
neuron_name = '20210113C'
singleneuron_data = SingleNeuron(neuron_name)

# singleneuron_data.plot_rawdatablocks(time_axis_unit='s', segments_overlayed=False)

# notes summary:
# quite nice recording with neuron keeping steady baselineV ~-50mV throughout, with quite a lot of spont.APs and a few
# fastevents; no response to light whatsoever.
...
# %% summary plots - neat events only: - all spont.events in this neuron are neat events
nbins = 100  #
...
# in the APs on currentpulsechange there is one that comes right between applied current pulses; labeling it as spont.:
# spontap = (aps_oncurrentpulsechange & (des_df.file_origin == 'gapFree_0000.abf')
#            & (des_df.peakv_idx > (20000 * 190)) & (des_df.peakv_idx < (20000 * 195)))
# singleneuron_data.depolarizing_events.loc[spontap, 'event_label'] = 'actionpotential'
# spont.APs all got picked up nicely; no light-evoked APs recorded in this neuron.
...
# notes:
# no events got picked up - unsurprising given that there did not seem to be any response to light.
...
# notes:
# just 6 events got picked up, looks like they could all be fastevents.
...
#                                    plot_dvdt=True
#                                    )
# Indeed, all fastevents except one: the single smallest-amp event (~2mV) also stands out for having
# short rise-time (0.2 vs 0.5-0.6ms) and way short half-width (<1.2 vs >3ms for all other events).
# Labeling all the others as fastevents:
# singleneuron_data.depolarizing_events.loc[(unlabeled_spont_events & (des_df.amplitude > 3)),
#                                           'event_label'] = 'fastevent'
# singleneuron_data.write_results()

#### -- this concludes sorting through all sub-threshold events and labeling them -- ####
# %% marking 'neat' events: events occurring during stable and 'good-looking' periods of recording
# I honestly see no real signs of deterioration in the recording; three APs with the lowest amp
# (88-89mV, vs 92-99mV for all others) occur towards the end of recording but are not the last APs to be fired.
# So I'll label the entire recording as neat:
