...
neuron_name = '20200630A'
singleneuron_data = SingleNeuron(neuron_name)

singleneuron_data.plot_rawdatablocks(time_axis_unit='s', segments_overlayed=False)

# notes summary:
# pretty nice recording, though baselineV a little more variable than I'd like - going down from ~-55 to -60mV at first,
# then depolarizes again and settles ~-50mV for two blocks of light pulses (and then leaves suddenly).
# Did not see any spont.fastevents or APs, but light seems to evoke fast things (and the occasional AP).
...
# light-evoked APs were picked up nicely; other types of APs not recorded (no DC pulses applied to see APs)
...
# notes:
# all light-evoked events got picked up as such, definitely nothing that's actually spontaneous labeled as evoked.
...
# notes:
# everything that got picked up as spontaneous events are in fact noise-things or oscillation peaks occurring
# during the first stretch of recording before neuron settles for a bit. Labeling them as such:
