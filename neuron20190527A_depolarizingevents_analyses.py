...
neuron_data = SingleNeuron('20190527A')
...
spontevents = (~neuron_data.depolarizing_events.applied_ttlpulse) & (~excludedevents)

allevents_df = neuron_data.depolarizing_events[~excludedevents]
evokedevents_df = neuron_data.depolarizing_events[evokedevents]
spontevents_df = neuron_data.depolarizing_events[spontevents]
largeevents_df = neuron_data.depolarizing_events[largeampevents]
largespontevents_df = neuron_data.depolarizing_events[spontevents & largeampevents]
largeevokedevents_df = neuron_data.depolarizing_events[evokedevents & largeampevents]

# %% Q0: did the algorithm for extracting depolarizing events actually work well?
neuron_data.plot_blocks_byname('light_0',
                               # segments_overlayed=False,
                               events_to_mark=evokedevents_df)

# neuron_data.plot_allrawdata(segments_overlayed=False, events_to_mark=spontevents_df)
# %% Q1: can different groups of depolarizing events be identified in the neuron's spontaneously occurring activity?
neuron_data.plot_depolevents_overlayed((largeampevents & spontevents),
                                       do_baselining=True,
                                       # do_normalizing=True,
                                       colorby_measure='baselinev',
                                       total_plotwindow_inms=20)
# signatures of fast-events:
#1. identical rise-time and decay
#2. different amplitude-groups
#3. slower decay for more hyperpolarized V

# use these signatures to find events that are DEFINITELY fast-events and label them as such;
# then use these (and only these) to compare evoked decays to.


# %% Q2: can fast-events be isolated from the neuron's response to excitatory inputs?
